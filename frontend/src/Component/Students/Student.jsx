...
const StudentDashboard = () => {
 ...
 // const roleId = localStorage.getItem("roleId"); // or "userId" depending on your naming

  if (!token) {
    toast({ title: "Login required to enroll", status: "warning" });
    return;
  }

  try {
    const response = await axios.get(
      `https://skillgate.onrender.com/course/enrollCourse/${courseId}`,
    ...
    toast({
      title: response.data.message || "Enrolled successfully",
      status: "success",
    });
  ...
  return (
    <Flex direction="column" h="100vh" w="100vw" bg="gray.50" overflow="hidden">
      <StudentNavbar onSearch={setSearchQuery} />
      <Flex
